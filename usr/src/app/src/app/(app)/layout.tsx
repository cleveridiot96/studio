...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [fabPosition, setFabPosition] = useLocalStorageState({ x: 0, y: 0 }, 'calculatorFabPosition');
  ...
  useEffect(() => {
    ...
    // It's safe to access `window` here.
    setIsAppLayoutMounted(true);
    // Set initial positions based on window size only after mount
    setCalculatorPosition(prev => ({
        x: prev.x === 0 ? window.innerWidth - 420 : prev.x,
  ...
  if (!isAppLayoutMounted) {
    return null; // Return null on the server and during initial client render to avoid hydration mismatches
  }
