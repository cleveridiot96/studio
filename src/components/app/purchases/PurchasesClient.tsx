...
const rawInitialPurchases: (Omit<Purchase, 'totalAmount' | 'effectiveRate' | 'transportRate'> & { transportRatePerKg?: number, expenses?: number, rate: number, netWeight: number, quantity: number })[] = [
  ...
  },
  {
    ...
    quantity: 50, netWeight: 2500, rate: 25, expenses: 200, transportRatePerKg: 0, 
  ...
  },
  {
    ...
    quantity: 75, netWeight: 3750, rate: 20, expenses: 0, transportRatePerKg: 0, 
  ...
  },
  {
    ...
    quantity: 120, netWeight: 6000, rate: 23.5, expenses: 0, transportRatePerKg: 0.1, 
    transporterId: "trans-reliable", transporterName: "Reliable Transports",
    locationId: "wh-pune", locationName: "Pune North Godown"
  },
  {
    id: "purchase-1", date: "2025-05-01", lotNumber: "AB/6", locationId: "w1", locationName: "Mumbai Godown",
    supplierId: "s1", supplierName: "AR Agent Supplier", agentId: "a1", agentName: "AR Agent",
    quantity: 6, netWeight: 300, rate: 320, expenses: 1000, transportRatePerKg: (2000 / (6*50)), 
    transporterId: "t1", transporterName: "Speedy Logistics",
  },
];
...
export function PurchasesClient() {
  ...
  // Recalculate effectiveRate for items loaded from localStorage if they don't have it
  React.useEffect(() => {
    if (purchases.some(p => p.effectiveRate === undefined || p.totalAmount === undefined || p.transportRate === undefined)) {
      console.log("Found purchases missing calculated fields. Upgrading...");
      setPurchases(prevPurchases => prevPurchases.map(p => {
        if (p.effectiveRate === undefined || p.totalAmount === undefined || p.transportRate === undefined) {
          const rawP = { ...p, transportRatePerKg: p.transportRatePerKg }; // ensure all needed fields are present for calc
          return calculatePurchaseEntry(rawP as any); // Cast to any if types conflict due to missing fields
        }
        return p;
      }));
    }
  ...
  const handleDownloadPurchasePdf = React.useCallback((purchase: Purchase) => {
    // Placeholder for PDF generation logic for a single purchase
    // This would typically involve a library like jsPDF and html2canvas,
    // and a specific printable template for the purchase chitti.
    console.log(`Placeholder: Download PDF for purchase ID: ${purchase.id}`);
    toast({
      title: "PDF Download (Placeholder)",
      description: `PDF for purchase ${purchase.lotNumber} would be generated here.`,
    });
  }, [toast]);
