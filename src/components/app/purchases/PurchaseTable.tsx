    ...
    <TooltipProvider>
      <ScrollArea className="rounded-md border shadow-sm">
        <Table className="min-w-full whitespace-nowrap">
          ...
          <TableBody>
            {data.map((purchase) => {
              const expensePerKg = purchase.netWeight && purchase.netWeight > 0 
                ? ((purchase.expenses || 0) / purchase.netWeight)
                : 0;
                  ...
                  <TableCell className="text-right">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span>{(purchase.expenses || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>Expense per kg: ₹{expensePerKg.toFixed(2)}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TableCell>
