...
interface AddSaleFormProps {
  ...
  brokers: MasterItem[];
  inventoryLots: Purchase[]; // Purchases act as inventory lots
  existingSales: Sale[]; // For stock calculation
...
const AddSaleFormComponent: React.FC<AddSaleFormProps> = ({
  ...
  const methods = useForm<SaleFormValues>({
    resolver: zodResolver(saleSchema(customers, transporters, brokers, inventoryLots, existingSales)),
  ...
  const brokerageValue = watch("brokerageAmount") || 0; // This is the input for rate/fixed amount
  ...
  const totalAmountForCustomer = finalBillAmountToUse; //This might change if other charges are added directly to customer bill

  // For profit calculation (conceptual, as purchase cost isn't fully plumbed here yet)
  // const purchaseCostOfGoodsSold = ...; 
  // const profit = totalAmountForCustomer - purchaseCostOfGoodsSold - transportCost - calculatedBrokerageCommission;


  const handleAddNewMasterClicked = (type: MasterItemType) => {
    toast({ title: "Add New Master", description: `Conceptual: Open form for ${type}. Please use Masters page.` });
  };
  ...
  const processSubmit = (values: SaleFormValues) => {
    ...
    const saleData: Sale = {
      ...
      billAmount: values.billAmount, // Store manual override if present
      ...
      brokerageAmount: values.brokerageAmount, // Storing the input value (rate/fixed)
            ...
            <form onSubmit={methods.handleSubmit(processSubmit)} className="space-y-4 max-h-[80vh] overflow-y-auto p-1 pr-3">
              ...
              <div className="p-4 border rounded-md shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField control={control} name="lotNumber" render={({ field }) => (
                    ...
                    <MasterDataCombobox name="lotNumber" options={inventoryLots.map(p => ({ value: p.lotNumber, label: `${p.lotNumber} (${p.locationName} - ${p.quantity} bags)` }))} placeholder="Select Lot" onAddNew={() => toast({title: "Info", description: "Add new lots via Purchases page."})} />
                    <FormMessage /></FormItem>)}
                  />
                  <FormField control={control} name="customerId" render={({ field }) => (
                    <FormItem><FormLabel>Customer</FormLabel>
                    <MasterDataCombobox name="customerId" options={customers.map(c => ({ value: c.id, label: c.name }))} placeholder="Select Customer" onAddNew={() => handleAddNewMasterClicked("Customer")} />
                    <FormMessage /></FormItem>)}
                  />
                </div>
              ...
              <div className="p-4 border rounded-md shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                    <FormField control={control} name="transporterId" render={({ field }) => (
                        ...
                        <MasterDataCombobox name="transporterId" options={transporters.map(t => ({ value: t.id, label: t.name }))} placeholder="Select Transporter" onAddNew={() => handleAddNewMasterClicked("Transporter")} />
                        <FormMessage /></FormItem>)}
                    />
                    ...
                    <FormField control={control} name="brokerId" render={({ field }) => (
                        ...
                        <MasterDataCombobox name="brokerId" options={brokers.map(b => ({ value: b.id, label: b.name }))} placeholder="Select Broker" onAddNew={() => handleAddNewMasterClicked("Broker")} />
                        <FormMessage /></FormItem>)}
                    />
              ...
              <div className="p-4 border border-dashed rounded-md bg-muted/50 space-y-2">
                ...
                {calculatedBrokerageCommission > 0 && (
                 ...
                 {/* Conceptual Profit Display - requires purchase cost data for the lot */}
                 {/* <p className="text-sm text-muted-foreground pl-7">Estimated Profit: <span className="font-semibold">₹{profit.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></p> */}
              </div>
