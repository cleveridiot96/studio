...
export function SalesClient() {
  ...
  const handleAddOrUpdateSale = React.useCallback((sale: Sale) => {
    ...
    removeLedgerEntries(sale.id); // Remove old entries first
    if (sale.expenses && sale.expenses.length > 0) {
      const newLedgerEntries = sale.expenses
        ...
        .map(exp => ({
            ...
            linkedTo: { voucherType: 'Sale' as const, voucherId: sale.id },
            remarks: `Expense for sale to ${sale.customerName}`
        }));
      if(newLedgerEntries.length > 0) addLedgerEntry(newLedgerEntries);
    }
    toast({ title: "Success!", description: isEditing ? "Sale updated." : "Sale added." });
    ...
    <div className="space-y-2 print-area">
      ...
      <Tabs defaultValue="sales" className="w-full" onValueChange={setActiveTab}>
        ...
        <TabsContent value="sales">
          ...
          <SaleTable data={paginatedSales} onEdit={handleEditSale} onDelete={handleDeleteSaleAttempt} onDownloadPdf={triggerDownloadSalePdf} currentPage={currentPage} totalPages={totalPages} goToPage={setCurrentPage} nextPage={() => setCurrentPage(p => p+1)} prevPage={() => setCurrentPage(p => p-1)} />
        </TabsContent>
